...
SAStreamer = 0.1*1;                     %Surface area of streamer [m^2]
...
CdStreamer = 0.4; % Coefficient of drag for streamer, online reference
...
elseif y(1) <= paraAlt || (y(1) <= burnAlt && y(4) <= 10) % Landing chute deployed, nose down, airbrake enabled, assume drag acts in both directions
    CdSA = CdFuse*SAFuse + CdChute*SAChute; % Sum of individual Cd*SA [m^2]
...
elseif y(1) <= burnAlt % Velocity burn off
    ...
    CdSA = CdFuse*SAFuse + CdWing*SAWing + CdStreamer*SAStreamer;               % Sum of individual Cd*SA [m^2]
