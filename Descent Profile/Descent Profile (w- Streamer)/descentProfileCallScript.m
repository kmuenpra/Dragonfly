...
[T, Y] = ode45(@(t,y) descentEoM(t,y,x), linspace(t0, tf, 10000), state0); % THE Y HERE IS THE FULL POSITION/VEL OUTPUT ARRAY
...
writematrix(Y, 'predictedTrajectory.dat')
% Return Values
...
for i = 1:pitchEndInd
    ...
    CdBrake = 1.29; % Coefficient of drag for airbrakes, flat plate
    CdStreamer = 0.4; % Coefficient of drag for Streamer, from online
    ...
    SAStreamer = 0.1*20; %Surface area of streamer [m^2]
    CdSA = CdFuse*SAFuse + CdStreamer*SAStreamer; % Sum of individual Cd*SA [m^2]
